...
const GlowCard: React.FC<GlowCardProps> = ({ children, identifier }) => {
  useEffect(() => {
    ...
    document.body.addEventListener('pointermove', (event) => UPDATE(event as MouseEvent));
    ...
    RESTYLE();
    UPDATE(null); // Initial update to apply styles

    // Cleanup event listener
    return () => {
      document.body.removeEventListener('pointermove', (event) => UPDATE(event as MouseEvent));
    };
  }, [identifier]);
